...
import sys
import threading
from typing import Any, Callable, Dict, Final, List, Optional, Sequence, Tuple, Union, TypedDict
...
class ConveyorBelt(EntityBase["ConveyorBelt"]):
    ...
    def set_target_speed(self, speed:float):
        ...
        """
        self._set_float("setTargetSpeed", float(np.clip(speed,-5.,5.)))
...
class RangeFinder(EntityBase["RangeFinder"]):
    ...
    def editor_set_can_read_rfid_tags(self, can_read_rfid:bool):
        """Can only be called from level editor. Enable or disable the rfid reader for this range finder.
        """
...
def show_nicegui(title = "My Nice GUI", width=800, height=600):
    ...
    ui.run(host="127.0.0.1",port=18085,show=False, dark=True, reload=False, viewport=f"width={width}, height={height}, initial-scale=1", title=title)
...
class FactBox(EntityBase["FactBox"]):
    ...
    def focus(self):
        ...
        """
        print("Just focusing the lootbox? Now that would be cheating wouldn't it? Try get_distance_to_player() instead.", Colors.Yellow)
...
class LevelEditor(EntityBase["LevelEditor"]):
    ...
    def spawn_entity(self, entity_type:SpawnableEntities, unique_name="", **kwargs):
        ...
        self._set_json("SpawnEntity", args | {
            "EntityClass":str(entity_type),
            "UniqueName":unique_name
            },True)

    def spawn_static_mesh(self, mesh:SpawnableMeshes, unique_name="", material=SpawnableMaterials.Default, simulate_physics=False, **kwargs):
        ...
        self._set_json("SpawnEntity", args | {
            ...
            "bPhysics":simulate_physics
            },True) #mark as placementspawn ingame

    
  
    #def set_level_metadata(self, name:str, description:str, category:str, )

    def specify_goal(self, unique_name:str, display_text:str, update_func:Callable[[str],None], goal_value:float=1.0, is_resettable:bool=True, is_optional:bool=False):
    ...
    def set_goal_progress(self, unique_name:str, new_progress:float, new_text:str = ""):
        ...
        """
        self._set_json("UpdateGoal", {
            "UniqueName":unique_name,
            "DisplayText":new_text,
            "CurrentValue":new_progress,
            "CurrentState":str(GoalState.Unknown)
            },True)
    def set_goal_state(self, unique_name:str, new_state:GoalState, new_text:str = ""):
    ...
    def set_rotation(self, unique_name:str, new_rotation:Tuple[float,float,float]):
        ...
        """
        self._set_json("SetRotation", {
            "UniqueName":unique_name,
            "Location":new_rotation
            }, True)
    ...
    def run_editor_level(self):
        ...
        from pyjop.Network import SimEnv
        if (LevelEditor._is_construct_only()) or (not self._dynamic_funcs and not self._goal_funcs):
            SimEnv.disconnect()
            return
        ...
        while SimEnv._is_connected:
            ...
            self._check_goals()
            #handle level reset
            if last_sim_time > m.get_sim_time() and self._on_level_reset_handler is not None:
