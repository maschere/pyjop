...
from typing import Any, Callable, Dict, Final, Generic, List, Optional, Sequence, Tuple, Type, TypeVar, Union
import numpy as np
import sys
import time
...
class NPArray:
    ...
    def from_msg(cls, msg_b):
        ...
        if msg_b[16] == 0:
            ...
            dt = np.float32
        name = str.strip(msg_b[NPArray._PRE_HEADER:NPArray._HEADER_SIZE].decode("ascii"))
...
class EventData:
    ...
    """
    def __init__(self, event_name:str, sender_uniquename:str, timestamp:float, data:Any) -> None:
...
class EventListener:
    def __init__(self, for_event:str, from_sender:str, is_oneshot=False, test_accept:Optional[Callable[[EventData],bool]] = None) -> None:
...
class EventHandler(EventListener):
    def __init__(self, handler_code: Callable[[EventData], None], for_event: str, from_sender: str, test_accept: Optional[Callable[[EventData], bool]] = None) -> None:
        super().__init__(for_event, from_sender, test_accept)
        self.handler_code = handler_code

    def clear(self):
        EntityBase._event_q_handlers = [h for h in EntityBase._event_q_handlers if h != self]
...
class EntityBase(Generic[T]):
    ...
    _event_q_raw: Queue[EventData] = Queue() #event name, sender unqiue name, timestamp, payload
    ...
    def _sync_incoming_data(nparr: NPArray):
        if nparr.unique_name.startswith("EventQ."):
            ...
            try:
                ...
                return
            EntityBase._event_q_raw.put_nowait(EventData(event_name, sender_name, ts, raw_payload))
    ...
    def find_all(cls, find_derived=False)->List[T]:
        ...
        if len(all_ents)==0:
            EntityBase._log_debug_static(f"Cannot find any entities of type '{cls.__name__}'", (1,0,0))
        return all_ents
    ...
    def find(cls, entity_name:str)->T:
        ...
        if fullname not in EntityBase._entity_dict:
            EntityBase._log_debug_static(f"Cannot find entity '{entity_name}'", (1,1,0))
            return None
        v = EntityBase._entity_dict[fullname]
        if isinstance(v, cls):
              return v
        EntityBase._log_debug_static(f"Cannot find entity '{entity_name}' of type '{cls.__name__}'", (1,1,0))
        return None
    ...
    def first(cls)->T:
        ...
        if (len(all)>0):
            return all[0]
        EntityBase._log_debug_static(f"Cannot find any entities of type '{cls.__name__}'", (1,1,0))
        return None
    ...
    def _get_string(self, prop_name:str) -> str:
        ...
        if k in self._in_dict:
            self._post_API_call()
            return self._in_dict[k].array_data.squeeze().tobytes().decode("ascii").strip() #check if ascii
        return ""
    ...
    def _set_int(self, prop_name:str, val:int, append:bool=False) -> None:
        k = self._build_name(prop_name)
        nparr = NPArray(k,np.frombuffer(int.to_bytes(val, 4, "little"),dtype=np.uint8))
        #check duplicate here
    ...
    def _get_image(self, prop_name:str, channels=3)->np.ndarray:
        k = self._build_name(prop_name)
        if k in self._in_dict and len(self._in_dict[k].array_data.shape)==3 and self._in_dict[k].array_data.shape[2]>=channels:
            if channels==3:
    ...
    def resend_data(self):
        ...
        self._set_void("*ResendData") #wildcard command for all components in this entity
        await_tick()
...
def _find_all_entity_classes(modules:List[ModuleType])->List[Type[EntityBase]]:
    ...
    for m in modules:
        ...
        all_classes = inspect.getmembers(m, lambda x: inspect.isclass(x) and issubclass(x, EntityBase) and x.__name__ not in builtin_classes)
        entity_classes.extend([i[1] for i in all_classes])
    return list(set(entity_classes))

def _find_all_entity_classes_rec(module:Optional[ModuleType]=None)->List[Type[EntityBase]]:
...
def _trace_debug_jop_call(frame:FrameType, event, arg):
    ...
    if finfo.code_context and (finfo.code_context[0].endswith('jopmainhidden()\n') or finfo.code_context[0].endswith('def jopmainhidden():\n')):
    ...
    if do_break and simenv:
        allvars = {k:str(v) for (k,v) in frame.f_locals.items() if not isinstance(v,(ModuleType))}
        _trace_debug_jop_call.print("DEBUG:", lineno, [s.strip() for s in finfo.code_context],allvars)
